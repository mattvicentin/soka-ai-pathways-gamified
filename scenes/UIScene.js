...
export class UIScene extends Phaser.Scene {
  constructor() {
    ...
    // R1 reflection completion flag (limits restart-only state)
    this.r1ReflectionCompleted = false;
    // Auto-advance tracking (nodes that should flow directly into R1)
    this.autoAdvanceAfterText = false;
    this.autoAdvanceChoice = null;
  }
  ...
  displayNode(node, retryCount = 0) {
    ...
    // Detect nodes whose only choices are Meta-Reflection + Restart (pre-R1 outcomes)
    if (node.id !== 'R1' && Array.isArray(node.choices) && node.choices.length) {
      const metaChoice = node.choices.find(choice =>
        choice.to === 'R1' ||
        (choice.label && choice.label.toLowerCase().includes('meta') && choice.label.toLowerCase().includes('reflection'))
      );
      const restartChoice = node.choices.find(choice =>
        choice.to === 'D1' ||
        (choice.label && choice.label.toLowerCase().includes('restart'))
      );
      if (metaChoice && restartChoice && node.choices.length === 2) {
        this.autoAdvanceAfterText = true;
        this.autoAdvanceChoice = metaChoice;
        console.log(`Auto-advance enabled for node ${node.id} -> ${metaChoice.to}`);
      }
    }
    ...
    // For R1, add a "Meta-Reflection" choice that will trigger the modal
    // This choice should appear alongside the "Restart" option
    if (node.id === 'R1' && node.choices) {
      // Check if Meta-Reflection choice already exists
      const hasMetaReflection = node.choices.some(choice => 
        choice.label.toLowerCase().includes('meta') && choice.label.toLowerCase().includes('reflection')
      );
      // Add Meta-Reflection choice if it doesn't exist
      if (!hasMetaReflection) {
        node.choices.unshift({
          label: 'Meta-Reflection',
          to: 'R1' // Stay on R1, but we'll handle this specially in selectChoice
        });
      }
    }
    
    // R1 should behave normally - show all text, then show choices
    // The reflection modal will be triggered by a choice selection, not automatically
    this.shouldShowReflection = false;
    
    // For R1, ensure all text displays without pagination breaks
    // Combine all text into a single page to prevent early choice appearance
    if (node.id === 'R1') {
      // Start typewriter with the full narrative as a single continuous text
      this.startTypewriter(node.narrative);
    } else {
      // Start typewriter effect for narrative (normal pagination)
      this.startTypewriter(node.narrative);
    }
  }
  ...
  splitTextIntoPages(text) {
    ...
    // Use an off-screen text object for precise measurement (same styling as dialogue text)
    ...
    const measureHeight = (content) => {
      ...
      // Height reflects wrapped text just like the on-screen narrativeText
      return tempText.height ?? 0;
    };
    
    // Preserve whitespace tokens (spaces/newlines) so formatting stays intact
    ...
    while (tokenIndex < tokens.length) {
      ...
      for (let i = tokenIndex; i < tokens.length; i++) {
        const candidate = pageText + tokens[i];
        const textHeight = measureHeight(candidate);
      ...
      // Always consume at least one token to avoid infinite loops with very long words
      if (consumedTokens === 0) {
        pageText = tokens[tokenIndex];
        consumedTokens = 1;
      }
      
      const trimmed = pageText.trim();
      pages.push(trimmed.length > 0 ? trimmed : pageText);
  ...
  startTypewriter(text) {
    ...
    // For R1, log pagination info for debugging
    if (this.currentNode && this.currentNode.id === 'R1') {
      console.log(`R1: Starting typewriter - ${this.textPages.length} page(s) total`);
      console.log(`R1: Full narrative text length: ${text.length} characters`);
    }
  ...
  finishTypewriter() {
    ...
    if (this.currentPage < this.textPages.length - 1) {
      ...
      // CRITICAL: Never show choices when there are more pages to display
      this.showContinueButton();
      console.log(`Page ${this.currentPage + 1}/${this.textPages.length} complete - showing continue button, NOT choices`);
      return; // Exit early - don't proceed to show choices
    } else {
      // All text pages displayed - this is the LAST page
      console.log(`All ${this.textPages.length} page(s) complete - ready to show choices`);
      ...
      // For R1, add extra validation to ensure ALL text is truly displayed
      if (this.currentNode && this.currentNode.id === 'R1') {
        // Verify we've completed all pages and all text is displayed
        const allTextDisplayed = this.currentPage >= this.textPages.length - 1 && 
                                 this.displayedText === this.fullText &&
                                 !this.isTyping;
        if (!allTextDisplayed) {
          console.log('R1: Not all text displayed yet, waiting...', {
            currentPage: this.currentPage,
            totalPages: this.textPages.length,
        ...
        console.log('R1: All text pages completed, ready to show choices');
      }
      
      // Handle auto-advance nodes (pre-R1 outcomes) before showing choices
      if (this.autoAdvanceAfterText && this.autoAdvanceChoice) {
        const nextNodeId = this.autoAdvanceChoice.to;
        const nextLabel = this.autoAdvanceChoice.label || 'Meta-Reflection';
        this.autoAdvanceAfterText = false;
        this.autoAdvanceChoice = null;
        console.log('Auto-advancing to final reflection node:', nextNodeId);
        this.showChoicesTimer = this.time.delayedCall(300, () => {
          this.selectChoice(nextNodeId, nextLabel);
          this.showChoicesTimer = null;
        });
        return;
      }
  ...
  continueToNextPage() {
    ...
    if (this.currentPage >= this.textPages.length - 1) {
      console.log('continueToNextPage: No more pages, should not be called');
      return;
    }
    
    // CRITICAL: Cancel any pending choice display timers when moving to next page
    // This prevents choices from appearing when there are more pages to show
    if (this.showChoicesTimer) {
      this.time.removeEvent(this.showChoicesTimer);
      this.showChoicesTimer = null;
      console.log('continueToNextPage: Cancelled pending choice display timer');
    }
    
    // Also clear any existing choices that might have appeared
    if (this.choiceButtons && this.choiceButtons.length > 0) {
      console.log('continueToNextPage: Clearing existing choices before next page');
      this.clearChoices();
    }
    ...
    // Move to next page
    this.currentPage++;
    console.log(`continueToNextPage: Moving to page ${this.currentPage + 1}/${this.textPages.length}`);
  ...
  showChoices() {
    ...
    if (!this.currentNode || !this.currentNode.choices || this.isTyping) {
      console.log('showChoices blocked: no node/choices or still typing');
      return;
    }
    
    // CRITICAL: Never show choices if there are more pages to display
    // This is the most important check - if we're not on the last page, don't show choices
    if (this.currentPage < this.textPages.length - 1) {
      console.log(`showChoices blocked: more pages remaining (page ${this.currentPage + 1}/${this.textPages.length})`);
      return;
    }
    
    // Don't show choices if reflection modal is open (for R1 node)
    if (this.currentNode.id === 'R1' && this.reflectionOverlay) {
      console.log('showChoices blocked: reflection modal is open');
      return;
    }
    
    // For R1, add extra validation to ensure ALL text is displayed
    if (this.currentNode.id === 'R1') {
      // Triple-check: ensure we've finished ALL pages and all text is displayed
      if (this.currentPage < this.textPages.length - 1) {
        console.log(`R1: Not showing choices - page ${this.currentPage + 1}/${this.textPages.length} (should be ${this.textPages.length}/${this.textPages.length})`);
        return;
      }
      if (this.isTyping) {
        console.log('R1: Not showing choices - still typing');
        return;
      }
      // Additional check: ensure displayed text matches full text
      if (this.displayedText !== this.fullText) {
        console.log('R1: Text not fully displayed yet', {
          displayedLength: this.displayedText.length,
          fullTextLength: this.fullText.length
        });
        return;
      }
      console.log('R1: All checks passed, showing choices');
    }
    
    // Always clear any existing choice renderings to avoid duplicates
    this.clearChoices();
    
    // Determine which choices should be shown
    let choicesToDisplay = this.currentNode.choices || [];
    if (this.currentNode.id === 'R1' && this.r1ReflectionCompleted) {
      choicesToDisplay = choicesToDisplay.filter(choice =>
        choice.to === 'D1' || (choice.label && choice.label.toLowerCase().includes('restart'))
      );
    }
    
    if (!choicesToDisplay.length) {
      console.log('showChoices: no choices available to display');
      return;
    }
    ...
    const totalChoices = choicesToDisplay.length;
    ...
    choicesToDisplay.forEach((choice, index) => {
      ...
      this.time.delayedCall(10, () => {
        ...
        hitArea.on('pointerdown', () => {
          this.selectChoice(choice.to, choice.label);
        });
  ...
  closeReflectionModal() {
    ...
    // After closing reflection modal on R1, mark completion and re-render choices
    if (this.currentNode && this.currentNode.id === 'R1') {
      this.r1ReflectionCompleted = true;
    }
  ...
  selectChoice(nextNodeId, choiceLabel) {
    console.log('Selected choice, going to node:', nextNodeId, 'label:', choiceLabel);
    ...
    // If we're on R1 and the choice is "Meta-Reflection" or "Meta‑Reflection", show modal immediately
    if (this.currentNode && this.currentNode.id === 'R1') {
      const normalizedLabel = choiceLabel ? choiceLabel.replace(/\u2011/g, '-').toLowerCase() : '';
      if (normalizedLabel.includes('meta') && normalizedLabel.includes('reflection')) {
        console.log('R1: Meta-Reflection choice selected, showing reflection modal');
        // Mark reflection as completed and refresh choices so only restart remains
        this.r1ReflectionCompleted = true;
        this.showChoices();
